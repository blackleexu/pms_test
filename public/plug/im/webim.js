...
(function(webim) {
        ...
        //最大允许长轮询返回错误次数
        var WEB_IM_LONG_POLLING_MAX_RET_ERROR_COUNT=150;
        var last_longPolling_time = 0;
	...
	var proto_sendMsg = function(msg, cbOk, cbErr) {
		...
		if (!checkLogin()){
            webim.onLongPollingStep('long_no_login');
            return;
        }
	...
	var ConnManager = lowerBR == false 
		...
		this.apiCall = function(type,cmd, data, cbOk, cbErr) {
                        ...
                        if(cmd == 'longpolling'){
                            webim.onLongPollingStep('long_script_req','data:'+JSON.stringify(data));
                        }
                        script.onload = script.onreadystatechange = function () {
                            ...
                            if(cmd == 'longpolling'){
                                webim.onLongPollingStep('long_end','data:'+JSON.stringify(data)+',resp:'+JSON.stringify(resp));
                            }
                            ...
                            if (resp && resp.ActionStatus == WEB_IM_ACTION_STATUS_OK){
                                console.info("["+type+"]["+cmd+"]success:\nurl:\n%s\ndata:\n%s\nresponse:\n%s",url,JSON.stringify(data),JSON.stringify(resp));
                                cbOk && cbOk(resp);
                            }else{
                                try{
	...
	var MsgManager = new function() {
		...
		var onGroupInfoChangeCallback = null;//群资料变化回调
                ...
                var longPolling=function(cbOk,cbErr){
                    last_longPolling_time = new Date().getTime();
                    webim.onLongPollingStep('long_start',last_longPolling_time);
                    var opts={
                        'Timeout':90,
                        'Cookie':{
                            'NotifySeq':notifySeq,
                    ...
                    proto_longPolling(opts,function(resp) {
                        webim.onLongPollingStep('long_end', ',resp:'+JSON.stringify(resp));
                        ...
                        webim.onLongPollingStep('long_end_error', ',resp:'+JSON.stringify(err));
	...
	webim.init = function(loginInfo, listeners, options) {
        ...
        webim.onLongPollingStep = function (step,data) {
            if(listeners.onLongPollingStep){
                listeners.onLongPollingStep(step,data);
            }
        };
