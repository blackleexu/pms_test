...
;(function(){
    ...
    C.prototype.init = function(){
        ...
        m.initstring = function()
        {
            ...
            $open_widget = $('<div class="open_widget '+zdy_class+'" id="'+_this.options.id+'">' +
                                    '<div class="ow_open_box" style="display: none;">' +
                                        '<div class="ow_open_close close" style="position: absolute;right: 10px;top: 10px;font-family: arial, sans-serif; cursor:pointer; display: inline-block; width: 30px; height: 30px; font-size: 30px; ; color: #666666; line-height: 30px; text-align: center; ">×</div>' +
        ...
        m.initPosition = function(){
            ...
            var pl, pt;
            pl = $doc.scrollLeft() + (($win.width() - 300) / 2);
            ...
            pl = $doc.scrollLeft() + ($win.width() - _this.options.width) / 2;
        ...
        m.anim = function(){
            var winhalf = ($win.height()) / 2;
            var pt = $doc.scrollTop() - _this.$ow_open_box.height() / 2 + winhalf;
            if(pt < 0){
                pt = 0;
            }
            if(_this.options.isFromR == true){
                ...
                _this.$ow_open_box.show().animate({top: pt, opacity: 1}, 500, 'easeOutBack', function () {
                    if (typeof _this.options.success == 'function') {
                        _this.options.success();
                        _this.is_loaded = true;
                        pop_up_num++;
    ...
    {
        var winhalf = ($win.height()) / 2;
        var pt = $doc.scrollTop() - this.$ow_open_box.height() / 2 + winhalf;
        if(pt < 0){
            pt = 0;
        }
        var pl = $doc.scrollLeft() + (($win.width() - this.$ow_open_box.width()) / 2);
    ...
    function c_confirm(content,fn,cancelfn,title)
    {
        var c = new C({
            ...
            btnString:'<div style="float: right;" class="btnCenter">' +
                            '<button class="btn btn-grey btn-sm cancel" style="float:left;margin: 5px;">'+t('cancel')+'</button>' +
                            '<button style="margin: 5px;float:left;" class="btn btn-sm btn-primary confirm">'+t('ok')+'</button>' +
                        '</div>',
            closefn:cancelfn
        });
