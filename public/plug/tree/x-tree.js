...
(function ($) {
    ...
    tree.prototype = {
        ...
        _checkTreeByIds: function (tree, sel_ids) {
            if (!Array.isArray(sel_ids)) {
                sel_ids = sel_ids.toString();
                sel_ids = sel_ids.replace(/(^\,*)|(\,*$)/g, "");
                if(sel_ids.indexOf(',') > 0){
        ...
        _makeTreeHeader: function (tree) {
            ...
            if(that._isIe()){
                ...
                var height =$wrap.height() - 4;
                if ($('.whiteDiv').length == 0 ) {
                    var whiteDiv = '<span class="whiteDiv" ' +
                        'style="display: inline-block;position: absolute;right: 5px;top: 5px;height:19px ;background: #fff;width: 20px"></span>';

                    $wrap.append(whiteDiv);
                }
            }
        ...
        _makeTreeFooter: function (tree) {
            ...
            $footer.$cancelAll.on('click', function name(params) {
                that.tree.$header.$input.val('');
                that.tree.$body.$children.show();

                that.cancelAll();
            });

            $footer.$confirm.on('click', function name(params) {
                that.opt.onConfirm.call(that,that.tree.$header.$input.val());
            });
        ...
        _makeSearchInput: function (item) {
            var $input = $('<input class="x-tree-search-input" type="text" placeholder="请输入关键词" ><div class="input_cover_fork"></div>');
            $input.css({
                'display': 'block',
                'width': '100%',
                'line-height': '30px',
        ...
        _makeIcon: function (item) {
            //添加自定义图标
            if ((!item || !item.is_node)&& !item.textIcon) {
                return '';
            }
            var textIcon = 'icon-2-filefolder';

            if(item.textIcon){
                textIcon = item.textIcon;
            }
            var $icon = $('<i class="iconfont '+textIcon+'"></i>');
            $icon.css({
                display: 'inline-block',
                'vertical-align': 'base-line',
                'font-size': '12px',
        ...
        _showTree: function () {
            ...
            if (this.opt.is_trigger) {
                ...
                if(this.opt.layout == 'top'){
                    cssStyle['bottom'] = this.dom.outerHeight();
                    // cssStyle['position']='relative';
                }else{
                    cssStyle['top'] = this.dom.outerHeight();
                }
                this.tree.$dom.css(cssStyle);
